              ...
              {service.features.map((feature, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`neon-card p-6 rounded-xl border border-neon-blue/30 hover:border-neon-blue/50 transition-all duration-300 group h-full flex flex-col cursor-pointer relative overflow-hidden ${shouldSpan ? 'md:col-span-2 lg:col-span-3' : ''}`}
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {service.benefits.map((benefit, index) => (
                <motion.div
                  ...
                  transition={{ duration: 0.6, delay: index * 0.1 }}
                  whileHover={{ y: -8, scale: 1.02 }}
                  className="neon-card p-6 rounded-xl border border-neon-purple/30 hover:border-neon-purple/50 transition-all duration-300 group h-full flex flex-col cursor-pointer relative overflow-hidden"
                >
                  <div className={`absolute inset-0 bg-gradient-to-br ${service.gradient} opacity-0 group-hover:opacity-10 transition-opacity duration-300`} />
                  <div className="relative z-10 flex flex-col h-full">
              ...
              {service.useCases.map((useCase, index) => {
                ...
                return (
                  <motion.div
                    ...
                    transition={{ duration: 0.6, delay: index * 0.1 }}
                    whileHover={{ y: -8, scale: 1.02 }}
                    className={`neon-card p-6 rounded-xl border border-neon-cyan/30 hover:border-neon-cyan/50 transition-all duration-300 group h-full flex flex-col cursor-pointer relative overflow-hidden ${shouldSpan ? 'md:col-span-2 lg:col-span-3' : ''}`}
                  >
                    <div className={`absolute inset-0 bg-gradient-to-br ${service.gradient} opacity-0 group-hover:opacity-10 transition-opacity duration-300`} />
                    <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="container-custom">
          <div className="max-w-4xl mx-auto">
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.6 }}
              className="relative p-10 md:p-14 rounded-3xl bg-gradient-to-br from-neon-cyan/20 via-neon-blue/15 to-neon-purple/20 border-2 border-neon-cyan/40 overflow-hidden backdrop-blur-sm"
            >
              ...
              <div className="relative z-10 text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Link
                    href="/contact"
                    className="btn-primary text-lg px-10 py-4 inline-block transform hover:scale-105 transition-transform duration-200 shadow-[0_0_30px_rgba(139,92,246,0.4)]"
                  >
                    Schedule a Consultation
                  </Link>
                  <Link
                    href="/contact"
                    className="btn-secondary text-lg px-10 py-4 inline-block transform hover:scale-105 transition-transform duration-200 border-2 border-neon-cyan/50 hover:border-neon-cyan hover:bg-neon-cyan/10"
                  >
                    Get in Touch
                  </Link>
                </div>
