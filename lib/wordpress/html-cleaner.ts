...
export function cleanWordPressContent(content: string): string {
  ...
  const openP = (cleaned.match(/<p[^>]*>/gi) || []).length;
  ...
  cleaned = cleaned.replace(/<h([1-6])[^>]*>([^<]*)<\/h([1-7])>/gi, (match, openLevel, content, closeLevel) => {
  ...
  cleaned = cleaned.replace(/(<\/p>)(<h[1-6])/gi, "$1\n$2");
  cleaned = cleaned.replace(/(<\/h[1-6]>)(<p)/gi, "$1\n$2");
  cleaned = cleaned.replace(/(<\/p>)(<ul|<ol)/gi, "$1\n$2");
  cleaned = cleaned.replace(/(<\/ul>|<\/ol>)(<p)/gi, "$1\n$2");
